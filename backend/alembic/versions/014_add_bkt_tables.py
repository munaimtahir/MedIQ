...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('p_S', sa.Float, nullable=False, comment='Probability of slip (learned but wrong)'),
        sa.Column('p_G', sa.Float, nullable=False, comment='Probability of guess (unlearned but correct)'),
        
        # Metadata
        sa.Column('constraints_applied', postgresql.JSONB, nullable=False, server_default='{}'),
        sa.Column('fitted_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()')),
        ...
        sa.Column('metrics', postgresql.JSONB, nullable=False, server_default='{}', comment='AUC, RMSE, logloss, CV metrics'),
        ...
        # Foreign keys
        sa.ForeignKeyConstraint(['algo_version_id'], ['algo_versions.id'], name='fk_bkt_skill_params_algo_version', ondelete='CASCADE'),
    )
    
    # Indexes for bkt_skill_params
    op.create_index('idx_bkt_skill_params_concept_active', 'bkt_skill_params', ['concept_id', 'is_active'])
    ...
    op.create_table(
        ...
        sa.Column('concept_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('p_mastery', sa.Float, nullable=False, server_default='0.0', comment='Current mastery probability'),
        ...
        sa.Column('algo_version_id', postgresql.UUID(as_uuid=True), nullable=True, comment='Version used for last update'),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()'), onupdate=sa.text('now()')),
        ...
        # Foreign keys
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='fk_bkt_user_skill_state_user', ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['algo_version_id'], ['algo_versions.id'], name='fk_bkt_user_skill_state_algo_version', ondelete='SET NULL'),
    )
    ...
    op.create_index('idx_bkt_user_skill_state_last_attempt', 'bkt_user_skill_state', ['last_attempt_at'])
    ...
    op.create_table(
        ...
        # Foreign keys
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='fk_mastery_snapshot_user', ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['algo_version_id'], ['algo_versions.id'], name='fk_mastery_snapshot_algo_version', ondelete='SET NULL'),
    )
    
    # Indexes for mastery_snapshot
    op.create_index('idx_mastery_snapshot_user_concept', 'mastery_snapshot', ['user_id', 'concept_id'])
...
def downgrade() -> None:
    ...
    op.execute("DELETE FROM algo_params WHERE algo_version_id IN (SELECT id FROM algo_versions WHERE algo_key = 'bkt')")
