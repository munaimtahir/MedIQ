...
def upgrade() -> None:
    ...
    op.add_column('attempt_events', sa.Column('event_version', sa.Integer, nullable=False, server_default='1'))
    op.add_column('attempt_events', sa.Column('client_ts', sa.DateTime(timezone=True), nullable=True))
    op.add_column('attempt_events', sa.Column('seq', sa.Integer, nullable=True))
    op.add_column('attempt_events', sa.Column('question_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('questions.id', onupdate='CASCADE'), nullable=True))
    op.add_column('attempt_events', sa.Column('source', sa.String(50), nullable=True))
    
    # Update payload_json to have default
    op.alter_column('attempt_events', 'payload_json',
