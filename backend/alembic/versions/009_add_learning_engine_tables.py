...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['algo_version_id'], ['algo_versions.id'], ondelete='CASCADE', onupdate='CASCADE'),
        sa.ForeignKeyConstraint(['created_by_user_id'], ['users.id'], onupdate='CASCADE'),
    )
    
    # Create indexes for algo_params
    op.create_index('ix_algo_params_algo_version_id_is_active', 'algo_params', ['algo_version_id', 'is_active'])
    ...
    op.create_index('ix_algo_runs_algo_version_id_started_at', 'algo_runs', ['algo_version_id', 'started_at'])
