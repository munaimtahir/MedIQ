...
from app.models.question_cms import Question as CMSQuestion, QuestionStatus, QuestionVersion, AuditLog
...
def test_submit_validation_fails_missing_fields(db: Session, admin_user: User):
    ...
    assert "year_id" in str(exc_info.value.detail) or "required" in str(exc_info.value.detail).lower()
...
def test_reviewer_cannot_publish(db: Session, reviewer_user: User, sample_question_data: QuestionCreate):
...
def test_publish_requires_source(db: Session, admin_user: User, sample_question_data: QuestionCreate):
...
def test_status_transition_invalid(db: Session, admin_user: User, sample_question_data: QuestionCreate):
...
def test_version_created_on_edit_and_status_change(db: Session, admin_user: User, sample_question_data: QuestionCreate):
...
def test_audit_log_written_on_create_update_publish(db: Session, admin_user: User, sample_question_data: QuestionCreate):
