...
class BKTSkillParams(Base):
    ...
    algo_version_id: Mapped[UUID] = mapped_column(
        ...
        ForeignKey("algo_versions.id", ondelete="CASCADE"),
        nullable=False
    )
    
    # BKT 4-parameter model
    p_L0: Mapped[float] = mapped_column(Float, nullable=False, comment="Prior probability of mastery")
    p_T: Mapped[float] = mapped_column(Float, nullable=False, comment="Probability of learning (transition)")
    p_S: Mapped[float] = mapped_column(Float, nullable=False, comment="Probability of slip (learned but wrong)")
    p_G: Mapped[float] = mapped_column(Float, nullable=False, comment="Probability of guess (unlearned but correct)")
    ...
    fitted_on_data_from: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    fitted_on_data_to: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    metrics: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict, comment="AUC, RMSE, logloss, CV metrics")
...
class BKTUserSkillState(Base):
    ...
    p_mastery: Mapped[float] = mapped_column(Float, nullable=False, default=0.0, comment="Current mastery probability")
