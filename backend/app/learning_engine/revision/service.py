...
async def compute_revision_queue_v0(
    ...
    for mastery in mastery_records:
        ...
        reason = {
            ...
            "last_attempt_days_ago": (current_date - mastery.last_attempt_at.date()).days if mastery.last_attempt_at else None,
            "is_due_now": is_due_now,
            "priority_breakdown": {
                "mastery_inverse": round((1.0 - mastery.mastery_score) * priority_weights.get("mastery_inverse", 70), 2),
                "recency": round(min((current_date - mastery.last_attempt_at.date()).days if mastery.last_attempt_at else 0, 90) * priority_weights.get("recency", 2), 2),
                "low_data_bonus": priority_weights.get("low_data_bonus", 10) if mastery.attempts_total < min_attempts else 0,
            },
        }
...
async def generate_revision_queue_v0(
    ...
    *,
    ...
    try:
        ...
        items = await compute_revision_queue_v0(
            db, user_id, params, current_date, year, block_id
        )
        ...
        # Upsert items
        num_upserted = await upsert_revision_queue(
            db, items, version.id, params_obj.id, run.id
        )
