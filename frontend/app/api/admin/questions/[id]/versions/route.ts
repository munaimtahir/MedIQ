/**
 * BFF route for question version history.
 */
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data } = await backendFetch<unknown>(`/admin/questions/${id}/versions`, {
      method: "GET",
    ...
    console.error("[Question Versions Route] Unexpected error:", error);
    return NextResponse.json(
      {
        error: {
          code: "INTERNAL_ERROR",
