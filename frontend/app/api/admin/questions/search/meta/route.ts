/**
 * API route for search metadata (proxies to backend)
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data } = await backendFetch<unknown>("/admin/questions/search/meta", {
      method: "GET",
    ...
    console.error("[Search Meta Route] Unexpected error:", error);
    return NextResponse.json(
      {
        error: {
          code: "INTERNAL_ERROR",
